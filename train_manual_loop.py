#!/usr/bin/env python3
"""Manual-loop LoRA SFT of Qwen3.5-4B (no Trainer / no accelerate / no DS).

Full control of the training loop:
- forward returns bf16 logits, chunked CE in fp32 (no giant .float())
- loss only on assistant segments (labels=-100 elsewhere)
- batch 1 x grad-acc 8, cosine lr, 3 epochs
"""
import json
import math
import os
...
OUTPUT_DIR = "checkpoints/qwen35-4b-lora-ds"
...
def tokenize(rendered, tok):
    ...
    if len(ids) > MAX_SEQ_LENGTH:
        # hard-cap BOTH sides so the total is exactly MAX_SEQ_LENGTH.
        # (old code kept head uncapped: a huge system+user could push the
        #  sequence to 30-60k tokens, making a single step take ~7 minutes)
...
def subvocab_ce(hidden, labels, lm_weight, tok_ignore=-100):
    """CE over only the vocab entries that appear in labels.

    hidden: [T, D] bf16 | labels: [T] long | lm_weight: [V, D] bf16 (detached)
    """
    pos = labels != tok_ignore
    uniq, inv = torch.unique(labels[pos], return_inverse=True)
    cls = torch.full_like(labels, -100)
    cls[pos] = inv
    logits = F.linear(hidden, lm_weight[uniq])  # [T, U] bf16
    shift = logits[:-1].contiguous()
    lb = cls[1:].contiguous()
...
def main():
    ...
    if os.path.exists(adapter_path):
        ...
        # NOTE: is_trainable=True is mandatory, otherwise all LoRA params
        # keep requires_grad=False and AdamW gets an empty parameter list.
    ...
    # transformer body + detached lm_head weight for sub-vocab loss
    orig = model.base_model.model
    body = orig.model
    lm_weight = orig.lm_head.weight.detach()

    # this arch's layers don't call checkpoint() themselves -> wrap manually
    ...
    for ep in range(start_ep, EPOCHS):
        ...
        while pos < len(order):
            ...
            if not loss.requires_grad:
                # chunk whose assistant tokens were fully truncated away:
                # constant-zero loss has no grad_fn -> skip it
    ...
    print("QWEN35-4B-LORA-MANUAL-DONE")
